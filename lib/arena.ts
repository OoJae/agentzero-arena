...
import type { ArenaState, EquityPoint, FinaleState } from "./types.js";
...
export function readArenaState(): ArenaState {
  ...
  const validation = getLatestValidationRuns(db);
  const finale = getFinaleState<FinaleState>(db);
  ...
  return { ts: Date.now(), agents, events, auditVerified, equitySeries, validation, finale };
}
